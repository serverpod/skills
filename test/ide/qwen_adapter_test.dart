...
import 'package:skills/src/ide/adapters/qwen_adapter.dart';
import 'package:test/test.dart';
import 'package:test_descriptor/test_descriptor.dart' as d;

void main() {
  group('Given a QwenAdapter', () {
    late QwenAdapter adapter;

    setUp(() async {
      await d.dir('project', [
        d.dir('.qwen', [d.dir('skills')]),
      ]).create();

      adapter = QwenAdapter(d.path('project'));
    });
...
'''),
      ...
      test('when installing then creates directory with skill name', () async {
        ...
        final installed = Directory(
          d.path('project/.qwen/skills/source_pkg-my-skill'),
        );
      ...
      test('when installing then copies SKILL.md as-is', () async {
        ...
        final skillMd = File(
          d.path('project/.qwen/skills/source_pkg-my-skill/SKILL.md'),
        );
      ...
      test('when installing then copies supporting directories', () async {
        ...
        final script = File(
          d.path('project/.qwen/skills/source_pkg-my-skill/scripts/run.sh'),
        );
        expect(await script.exists(), isTrue);

        final ref = File(
          d.path(
            'project/.qwen/skills/source_pkg-my-skill/references/guide.md',
          ),
        );
    ...
    group('and a previously installed skill', () {
      ...
      test('when removing then deletes the skill directory', () async {
        ...
        final removed = Directory(
          d.path('project/.qwen/skills/pkg-old-skill'),
        );
